...
class Tree:
    def __init__(self, model, name=None):
        self.model = model
        self.name = name or "Tree%s" % \
                    len(self.model.treelist.trees)
        self.wscale = 2.0
        self.scaled = False
    ...
    def errors(self):
        ...
        for n in self.root.iternodes():
            if not n.istip and len(n.children) != 2:
                v.append(n.label)
        if v:
            return "Tree is not dichotomous at nodes: %s" % ", ".join(v)
        leaves = [ lf.label for lf in self.root.leaves() ]
        keys = self.model.datamatrix.data.keys()
        v = [ lf for lf in leaves if lf not in keys ]
        if v:
            return "Cannot find species range data for %s" % ", ".join(v)
